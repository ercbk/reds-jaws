...
$\text{threshold} = \text{median} - 2*\text{MAD} = 3.0696$

$\text{threshold} = \text{1st quartile} - 1.5*\text{IQR} = 1$
...
$\text{score} = \frac{\text{player stat value} - \text{HOF stat median}}{\text{HOF stat MAD}}$
      
$\text{score} = \frac{\text{player stat value} - \text{HOF stat mean}}{\text{HOF stat sd}}$
...
kable(matrix(c(
      ...
      '4 yrs', '92%',
      NULL
), ncol = 2, byrow=TRUE, dimnames = list(NULL, c('Cutoff', '% Cutoff'))), format = "html",
booktabs=TRUE, caption = 'Inductees') %>%
      kable_styling() %>%
      row_spec(4, bold = T, color = "white", background = "#C6011F")
